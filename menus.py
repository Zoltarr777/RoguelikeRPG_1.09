...
def equipment_inventory_menu(con, header, equipment_inventory, equipment_inventory_width, screen_width, screen_height):
	if len(equipment_inventory.equipment_items) == 0:
		options = ["Equipment Inventory is empty."]
	else:
		options = [equipment_item.name for equipment_item in equipment_inventory.equipment_items]
...
def character_screen(player, character_screen_width, character_screen_height, screen_width, screen_height):
	...
	libtcod.console_print_rect_ex(window, 0, 11, character_screen_width, character_screen_height, libtcod.BKGND_NONE, libtcod.LEFT, "Gold:    {0}".format(player.fighter.gold))
