...
from components.equipment_inventory import Equipment_Inventory
...
def get_game_variables(constants):
	fighter_component = Fighter(hp=100, defense=1, power=2, magic=0, talismanhp=0, gold=0)
	inventory_component = Inventory(26)
	equipment_inventory_component = Equipment_Inventory(26)
	level_component = Level()
	equipment_component = Equipment()
	player = Entity(0, 0, '@', libtcod.white, 'Player', blocks=True, render_order=RenderOrder.ACTOR, fighter=fighter_component, inventory=inventory_component, level=level_component, equipment=equipment_component, equipment_inventory=equipment_inventory_component)
	entities = [player]

	equipment_component = Equippable(EquipmentSlots.MAIN_HAND, power_bonus=1)
	dagger = Entity(0, 0, '-', libtcod.darker_orange, "Dagger (+1 atk)", equippable=equipment_component)
	player.inventory.add_item(dagger)
	player.equipment.toggle_equip(dagger)

	item_component = Item(use_function=cast_magic, damage=2, maximum_range=3)
	magic_wand = Entity(0, 0, '|', libtcod.darker_sepia, "Magic Wand", item=item_component)
