...
from menus import main_menu, message_box, main_menu_help_menu
...
def play_game(player, entities, game_map, message_log, game_state, con, panel, constants):
	...
	while not libtcod.console_is_window_closed():
		...
		if move and game_state == GameStates.PLAYERS_TURN:
			...
			for entity in entities:
				if entity.item and entity.x == player.x and entity.y == player.y:
					...
					pickup_results = player.equipment_inventory.add_equipment_item(entity)
					player_turn_results.extend(pickup_results)
		...
		if equipment_inventory_index is not None and previous_game_state != GameStates.PLAYER_DEAD and equipment_inventory_index < len(player.equipment_inventory.equipment_items):
			equipment_item = player.equipment_inventory.equipment_items[equipment_inventory_index]
			player_turn_results.extend(player.equipment_inventory.use_equipment(equipment_item))
		...
		if cast_magic_wand and game_state == GameStates.PLAYERS_TURN:
			...
			if wand is None:
			    message_log.add_message(Message("You do not have a magic wand.", libtcod.orange))
			else:
		...
		if exit:
			if game_state in (GameStates.SHOW_INVENTORY, GameStates.DROP_INVENTORY, GameStates.CHARACTER_SCREEN, GameStates.HELP_MENU, GameStates.SHOW_EQUIPMENT_INVENTORY):
				game_state = previous_game_state
