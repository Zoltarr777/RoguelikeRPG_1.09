...
from item_functions import cast_confusion, cast_fireball, cast_lightning, heal, cast_sleep, cast_sleep_aura, health_talisman_sacrifice
from components.equipment_item_functions import equipment_heal
from map_objects.rectangle import Rect
from map_objects.tile import Tile
from components.equipment_item import Equipment_Item
...
class GameMap:
	...
	def place_entities(self, room, entities):
		...
		'''
		for i in range(number_of_equipment_items):
			x = randint(room.x1 + 1, room.x2 - 1)
			y = randint(room.y1 + 1, room.y2 - 1)

			if not any ([entity for entity in entities if entity.x == x and entity.y == y]):
				equipment_item_choice = random_choice_from_dict(equipment_item_chances)

				if equipment_item_choice == 'equipment_health_potion':
					equipment_item_component = Equipment_Item(use_equipment_function=equipment_heal, amount=60)
					equipment_item = Entity(x, y, '$', libtcod.black, 'Equipment Health Potion', render_order=RenderOrder.ITEM, equipment_item=equipment_item_component)

				entities.append(equipment_item)
		'''
