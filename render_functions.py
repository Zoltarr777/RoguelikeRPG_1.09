...
from menus import character_screen, inventory_menu, level_up_menu, bag_menu, help_menu, equipment_inventory_menu
...
def render_all(con, panel, entities, player, game_map, fov_map, fov_recompute, message_log, screen_width, screen_height, bar_width, panel_height, panel_y, mouse, colors, game_state):
    ...
    libtcod.console_print_ex(panel, 1, 3, libtcod.BKGND_NONE, libtcod.LEFT, "Gold: {0}".format(player.fighter.gold))

    if player.inventory.search("Health Talisman") is not None:
    	libtcod.console_print_ex(panel, 1, 4, libtcod.BKGND_NONE, libtcod.LEFT, "Talisman HP: {0}".format(player.fighter.talismanhp))
    ...
    if game_state in (GameStates.SHOW_INVENTORY, GameStates.DROP_INVENTORY):
        ...
        equipment_inventory_menu(con, "Press the key next to an item to equip it, or ESC to cancel.\n",
